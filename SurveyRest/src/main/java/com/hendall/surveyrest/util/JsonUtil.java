...
import com.hendall.survey.schematopojo.HelpSectionLink;
import com.hendall.survey.schematopojo.Question;
import com.hendall.survey.schematopojo.Section;
...
public class JsonUtil {
	...
	public static ClassName readFromFile(String is) {
		ClassName className = null;
		try {
			ObjectMapper objectMapper = new ObjectMapper();
			// objectMapper.enable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES);
			className = objectMapper.readValue(new File(is), ClassName.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return className;
